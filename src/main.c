...
void tokenize_and_fill_succs(char *delimiters, char *str) {
  ...
  while (token != NULL) {
    ...
    // If we have a previous token, add current token as successor
    if (prev_token != NULL) {
...
size_t random_token_id_that_starts_a_sentence() {
  // Keep trying until we find a token that starts with capital letter
  while (true) {
    size_t random_id = rand() % tokens_size;
    if (tokens[random_id] != NULL && isupper(tokens[random_id][0])) {
      return random_id;
    }
  }
}
...
char *generate_sentence(char *sentence, size_t sentence_size) {
  ...
  do {
    // Get number of successors for current token
    if (succs_sizes[current_token_id] == 0) 
    break; // No successors available
    
    // Pick a random successor
    char *next_token = succs[current_token_id][rand() % succs_sizes[current_token_id]];
    
    // Calculate length if we add this token
    sentence_len_next = strlen(sentence) + strlen(next_token) + 2; // +1 for space, +1 for null terminator
    if (sentence_len_next >= sentence_size) 
    break;
    
    // Add space and the next token
    strcat(sentence, " ");
    strcat(sentence, next_token);
    
    // Update current token for next iteration and stop if this token ends a sentence
